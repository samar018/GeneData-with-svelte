<script>
  ...
  // Declare the interactionsData prop
  export let interactionsData;

  let chartNode;

  // Function to create the hive plot
  function createHivePlot(interactionsData) {
      ...
      var svg = svgDOM
        ...
        .attr(
          "transform",
          "translate(" + outerRadius * 0.20 + "," + outerRadius * 0.57 + ")"
        );

      // Load the data and display the plot!
      var nodes = interactionsData;
  ...
  // Fetch the gene-gene interactions dataset and create the hive plot on component mount
  onMount(() => {
    createHivePlot(interactionsData);
  });
</script>
